...
client = MongoClient('mongodb://localhost:27017/')
client.filesys_database = {}
fs_db = client.filesys_database
fs_db.filenodes = {}
#Node_Contents = fs_db.filenodes
...
#        rpc.put(Binary(key), Binary(pickle.dumps(value)), 6000)
        dict_temp = {self.path : key , key: value}
		Node_Contents = fs_db.filenodes
		Node_id = Node_Contents.insert_one(dict_temp).inserted_id
...
#        res = rpc.get(Binary(key))
        Node_Contents = fs_db.filenodes
		res = Node_Contents.find_one({self.path:key})
		if key in res:
		    return res[key]
		else:
		    return None
